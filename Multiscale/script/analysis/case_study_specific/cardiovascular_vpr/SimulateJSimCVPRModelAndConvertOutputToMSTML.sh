...
function SimulateModel() {
    ...
    timeout ${COMMAND_EXECUTION_TIMELIMIT} ${JSIMHOME}/linux/bin/jsbatch -f ${MODEL_INPUT_FILE} -ofmt csv -out ${rawSimulationOutputPath} 1>${simulationLogOutputPath} 2>&1;
...
function ConvertModelSimulationOutputToCsv() {
    ...
    timeout ${COMMAND_EXECUTION_TIMELIMIT} cat ${rawSimulationOutputPath} | cut -d"," -f1,109,112,119,121,122,123,125,126 | awk --field-separator="," '/^[0-9.,]+/ { time=($1 * 1000); sub("[0-9E.]+", "", $0); print time $0; } /^time/ { print $0; }' > ${processedSimulationOutputPath};
