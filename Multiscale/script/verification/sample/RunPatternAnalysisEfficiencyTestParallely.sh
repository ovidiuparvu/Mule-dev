...
if [[ $# -ne 2 ]];
then
    echo "Usage: $0 <pattern-analysis-executable-path> <time-series-folder-path>.";
...
# Check if the time series folder path points to a directory
if [[ ! -d $2 ]];
then
    echo "The time series file path does not point to a directory. Please change.";
...
# time series folder and record the total execution time
#
###############################################################################

# Inform the user about the next action
echo "Running pattern analysis procedure for time series in the input folder $2...";
