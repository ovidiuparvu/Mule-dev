...
TEMPLATE_PREPROCESSING_IO = [
    (
        TEMPLATES_FOLDER_PATH + "/spatial_measure_type_hpp.tpl.in", 
        TEMPLATES_FOLDER_PATH + "/spatial_measure_type_hpp.tpl", 
        1, 
        29
    ),
    (
        TEMPLATES_FOLDER_PATH + "/subset_specific_type_hpp.tpl.in", 
        TEMPLATES_FOLDER_PATH + "/subset_specific_type_hpp.tpl", 
        29, 
        1
    ),
    (
        TEMPLATES_FOLDER_PATH + "/symbol_tables_auto_generated_hpp.tpl.in", 
        TEMPLATES_FOLDER_PATH + "/symbol_tables_auto_generated_hpp.tpl", 
        1, 
        1
    )
]
...
SOURCE_FILE_GENERATING_IO = {
    ...
    ,
    "spatial_measure_attribute_auto_generated_cpp.tpl" : 
        MODEL_CHECKING_SOURCE_FOLDER + str(
            "/attribute/SpatialMeasureAttributeAutoGenerated.cpp"
        )
    ,
    "spatial_measure_type_hpp.tpl" : 
        MODEL_CHECKING_INPUT_FOLDER + str(
            "/attribute/SpatialMeasureType.hpp"
        )
    ,
    "spatial_temporal_data_reader_auto_generated_cpp.tpl" : 
        MODEL_CHECKING_SOURCE_FOLDER + str(
            "/data/SpatialTemporalDataReaderAutoGenerated.cpp"
        )
    ,
    "subset_specific_attribute_auto_generated_cpp.tpl" : 
        MODEL_CHECKING_SOURCE_FOLDER + str(
            "/attribute/SubsetSpecificAttributeAutoGenerated.cpp"
        )
    ,
    "subset_specific_type_hpp.tpl" : 
        MODEL_CHECKING_INPUT_FOLDER + str(
            "/attribute/SubsetSpecificType.hpp"
        )
    ,
    "symbol_tables_auto_generated_hpp.tpl" : 
        MODEL_CHECKING_INPUT_FOLDER + str(
...
DERIVED_SPATIAL_ENTITY_OUTPUT_FOLDER = MODEL_CHECKING_INPUT_FOLDER + "/model/"

TIMEPOINT_TEMPLATE_PATH = "time_point_auto_generated_cpp.tpl"
TIMEPOINT_OUTPUT_PATH = MODEL_CHECKING_SRC_FOLDER + str(
    "/model/TimePointAutoGenerated.cpp"
)
...
def generate_time_point_file(environment, template_path, output_path, 
                             spatial_measures):
    ...
    is_density_spatial_measure = len(
        [spatial_measure for spatial_measure in spatial_measures 
            if spatial_measure.name == LABEL_DENSITY_SPATIAL_MEASURE
        ]
    ) > 0
    is_centroid_x_spatial_measure = len(
        [spatial_measure for spatial_measure in spatial_measures 
            if spatial_measure.name == LABEL_CENTROID_X_SPATIAL_MEASURE 
        ]
    ) > 0
    is_centroid_y_spatial_measure = len(
        [spatial_measure for spatial_measure in spatial_measures 
            if spatial_measure.name == LABEL_CENTROID_Y_SPATIAL_MEASURE
        ]
    ) > 0
    ...
    with open(output_path, "w+") as output_file:
        output_file.write(
            template.render({
                TAG_DENSITY_SPATIAL_MEASURE_FLAG : 
                    is_density_spatial_measure
                , 
                TAG_CENTROID_X_SPATIAL_MEASURE_FLAG : 
                    is_centroid_x_spatial_measure
                ,
                TAG_CENTROID_Y_SPATIAL_MEASURE_FLAG : 
                    is_centroid_y_spatial_measure
                ,
                TAG_WRN_AUTO_GENERATED_FILE : 
                    WRN_AUTO_GENERATED_FILE
            })
        )
...
def generate_source_files(spatial_entities, spatial_measures):
    ...
    generate_time_point_file(
        environment, TIMEPOINT_TEMPLATE_PATH, TIMEPOINT_OUTPUT_PATH, 
        spatial_measures
    )
