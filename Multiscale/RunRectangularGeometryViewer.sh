...
then
    ...
    echo
    echo "Usage:";
    echo "    RectangularGeometryViewer <path_to_csv_file> <nr_concentrations_for_each_position> <height> <width> [<selected-concentration-index>] [<use-log-scaling>] [<use-lexicographic-iterator>]";
...
bin/RectangularMapCsvToInputFiles --input-file "${INPUT_FOLDER}/${folderName}/${csvFileBasename}" --nr-concentrations-position ${nrOfConcentrationsForPosition} --height $height --width $width --output-file "${INPUT_FOLDER}/${folderName}/${csvFilename}"  ${selectedConcentrationIndex} ${useLogScaling} ${useLexicographicIterator};
