...
const std::string   CommandLineModelChecking::HELP_DESCRIPTION_MSG                                              = "    Mudi is a multidimensional (spatial-temporal) approximate probabilistic model checker. It can be used for two different types of applications. First of all Mudi can be employed to validate logic properties against multidimensional models. Secondly it can be used in reverse mode as a method to query time series data generated by in vivo/vitro experiments. Properties of interest are formalised using a spatio-temporal logic and their validity is checked using Mudi.";
