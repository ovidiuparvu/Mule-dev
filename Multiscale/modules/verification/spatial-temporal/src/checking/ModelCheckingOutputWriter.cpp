...
void ModelCheckingOutputWriter::printEvaluationResultsSummary(const std::size_t &logicPropertyIndex,
    ...
    std::size_t nrOfCorrectTraces  = 0;

    for (std::size_t i = 0; i != nrOfTraces; i++) {
        if (isTraceEvaluatedTrueForLogicProperty(logicPropertyIndex, i, evaluationResults)) {
            nrOfCorrectTraces++;
        }
    }

    printEvaluationResultsSummary(nrOfTraces, nrOfCorrectTraces);
}
...
const std::string ModelCheckingOutputWriter::MSG_INTRO_NAME                         = "Mudi 0.6.137 (Multidimensional model checker)";
